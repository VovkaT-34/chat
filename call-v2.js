...
(function () {
    ...
    function addButtons(){const header=document.querySelector(".chat-header-actions");if(!header)return;if(!$('callAudioButton')){const audio=document.createElement("button");audio.id="callAudioButton";audio.type="button";audio.title="Аудиозвонок";audio.setAttribute("aria-label","Аудиозвонок");audio.className="chat-call-button chat-call-audio";audio.innerHTML='<span class="call-icon-phone">☎</span>';audio.addEventListener("click",()=>void startCall("audio"));header.appendChild(audio);}if(!$('callVideoButton')){const video=document.createElement("button");video.id="callVideoButton";video.type="button";video.title="Видеозвонок";video.setAttribute("aria-label","Видеозвонок");video.className="chat-call-button chat-call-video";video.innerHTML='<span class="call-icon-video"></span>';video.addEventListener("click",()=>void startCall("video"));header.appendChild(video);}updateChatCallButtonVisibility();}
