...
async function enableChatPushNotifications() {
    ...
    return (await saveChatPushSubscription(subscription)) ? subscription : null;
}
...
async function sendChatPushForMessage(messageId) {
    ...
    try {
        const { data, error } = await window.supabaseClient.functions.invoke("send-message-push", {
            body: { messageId }
        });
...
function createPushButton() {
    ...
    button.type = "button";
    button.textContent = Notification.permission === "granted" ? "✅ Уведомления включены" : "🔔 Включить уведомления";
    button.style.cssText = [
        ...
        "background:linear-gradient(135deg,#171412,#735039)","color:#fffaf5","font-weight:700","font-size:13px",
        "box-shadow:0 5px 14px rgba(55,38,25,.2)","cursor:pointer"
    ].join(";");
    ...
    button.addEventListener("click", async () => {
        ...
        button.textContent = subscription ? "✅ Уведомления включены" : "🔔 Включить уведомления";
    });
