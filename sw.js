...
self.addEventListener("activate",event=>{
    event.waitUntil(
        caches.keys().then(keys=>Promise.all(
            keys.filter(key=>key.startsWith("chat-pwa-") && key!==CACHE_NAME).map(key=>caches.delete(key))
        ))
    );
    self.clients.claim();
});
...
async function confirmPushDelivery(data){
    ...
    try{
        await fetch(CONFIRM_DELIVERY_URL,{
            method:"POST",
            headers:{"Content-Type":"application/json","x-delivery-token":data.deliveryToken},
            body:JSON.stringify({messageId:data.messageId,userId:data.userId,exp:data.exp}),
            keepalive:true
        });
    }catch(error){
...
self.addEventListener("push",event=>{
    ...
    catch{ data={type:"message",title:"Чат",body:event.data?event.data.text():"Новое сообщение"}; }
    ...
    // Вызов получает отдельное уведомление. На поддерживаемых браузерах
    // доступны действия; на остальных обычный клик откроет страницу звонка.
    if(isCall && Array.isArray(Notification?.prototype?.actions)){
        options.actions=[
...
self.addEventListener("notificationclick",event=>{
    ...
    // Веб-приложение не может из Service Worker напрямую получить доступ
    // к микрофону/камере. Поэтому действие "Ответить" открывает приложение
    // с параметром вызова, после чего call-v2.js принимает управление.
    let targetUrl=data.url||"./index.html";
    if(data.type==="incoming-call" && data.callId){
        const separator=targetUrl.includes("?")?"&":"?";
        targetUrl+=`${separator}incomingCall=1`;
    }
